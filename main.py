...
async def _start_http_server(application):
    http_app = web.Application()
    http_app.router.add_get("/", _health_handler)
    http_app.router.add_get("/health", _health_handler)
    ...
    logger.info("Render health server 0.0.0.0:%s portda ishga tushdi.", port)
...
async def _post_init(application):
    global _AUTO_BACKUP_WAKE_EVENT
    _AUTO_BACKUP_WAKE_EVENT = asyncio.Event()
    await _start_http_server(application)
